...
# is outputting audio). Then, change the right dropdown to be "combined_output".
#
...
pactl load-module module-null-sink sink_name=recorded_rink sink_properties=device.description=recorded_sink
pactl load-module module-loopback source=$MIC_SOURCE sink=combined_output
pactl load-module module-loopback source=$UNUSED_OUTPUT sink=combined_output
pactl load-module module-loopback source=$UNUSED_OUTPUT sink=$SPEAKER_SINK
